...
2. 新用户 6 步研究导航、跳过与随时重开指南。
3. 动态项目路线与可调整的进度、材料、截止日期和推进模式。
4. 用户独立的研究项目 Dashboard。
5. 学术画像、多文档和可拖拽章节树。
6. 多格式材料上传、预处理、OCR 与 AI 语义解析。
7. 解析质量清单、动态 readiness 和材料补全。
8. 材料不足时检索真实文献候选并回传原文。
9. 项目知识库和文档专属材料范围。
10. 章节生成、选区共写、预览和选择性应用。
11. 可信链、原创实证风险、审查、申诉与复查。
12. 整篇质量聚合和 DOCX/PDF 导出。
...
1. 新用户登录且没有项目时，应自动进入 `/app/onboarding`。
2. 检查“研究准备 / 研究资产 / 写作交付”三阶段说明。
3. 依次填写项目、研究方式、当前进度、已有内容和截止日期。
4. 在第 6 步确认个性化路线，再创建首个项目。
5. 另一轮可点击“稍后再说”，确认进入项目列表且刷新后不再次自动弹出。
6. 从顶栏点击“使用指南”，确认仍可随时查看只读系统导览。

## 5. 推荐业务演示

1. 新用户通过 6 步研究导航创建首个项目；已有用户也可在 Dashboard 直接新建。
2. 进入项目概览，检查 7 项动态路线、当前任务、任务依据和预期产出。
3. 点击“调整路线”，修改当前进度、已有内容、截止日期或推进模式。
4. 上传学校/导师/期刊要求、研究笔记、数据、已有草稿和参考文献。
5. 进入解析页，执行预处理和 AI 解析。
6. 查看“可用于生成 / 建议确认 / 需要补充 / 解析失败”质量状态。
7. 对问题使用“填入补充说明”并重新解析。
8. 进入材料检查；材料不足时打开文献补充入口。
9. 检索 Crossref / OpenAlex / Semantic Scholar，查看候选质量、匹配原因与元数据缺口。
10. 将候选加入待下载清单，自行获取原文后回到上传页关联候选。
11. 原文完成 AI 解析后重新执行 readiness。
12. 构建知识库并进入“学术文档”。
13. 切换文档、拖拽章节、设置文档材料范围。
14. 生成或编辑章节，选择一段正文生成共写预览。
15. 演示整版、逐段或差异行应用，以及基准版本变化后的 `409` 防覆盖。
16. 打开本章检查，查看可信链、原创补强和审查项。
17. 手动审查并复查单项问题。
18. 切换整篇检查，从问题跳回对应章节。
19. 组装并导出文档；整篇视图不直接编辑全文。
...
- 首次向导为 6 个固定步骤，不调用 AI，也不会增加模型成本。
- 路线只推荐下一步，不锁定页面；知识库明确标记为推荐步骤。
- 任务状态来自真实材料、解析、准备度、章节和审查数据，不由前端猜测。
- “为什么需要”和“预期产出”按需展开，避免项目概览再次变得拥挤。
