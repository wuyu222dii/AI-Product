...
### 共写预览职责
- 生成共写候选正文
- 生成段落级 diff、冲突提示和复查建议
- 只在用户确认后创建新版本
- 应用预览后刷新审查项与可信链
- 保存共写预览与可能受影响审查项的关联关系
...
- `draft_generate`
- `co_write_preview`
- `co_write_preview_apply`
- `co_write_preview_discard`
- `rewrite_selection`
- `add_evidence`
- `adjust_structure`
- `reduce_repetition`
...
sequenceDiagram
    ...
    participant EVI as Evidence Binding Service

    U->>FE: 发起重写 / 补证据 / 调结构
    FE->>API: POST /workspaces/{id}/co-write/preview
    API->>DFT: 生成候选正文与 diffSummary
    DFT->>REV: 查询待处理审查项并保存关联关系
    DFT-->>API: CoWritePreview
    API-->>FE: 展示差异、冲突提示和复查建议
    U->>FE: 逐段接受或应用为新版本
    FE->>API: POST /co-write-previews/{id}/apply
    API->>DFT: 创建新 Draft Version
    DFT->>REV: 刷新审查项
    DFT->>EVI: 重建可信链
    API-->>FE: 刷新工作台
```
...
sequenceDiagram
    ...
    FE->>API: GET /drafts/{id}/evidence-bindings
    API->>EVI: 读取覆盖率与引用一致性
    EVI-->>API: Evidence Summary
    API-->>FE: 展示导出前风险清单
    U->>FE: 点击导出
