...
当前结论：项目保留 `v1.x MVP + v1.8 + v1.9` 完整能力，并完成 `v2.0 全学术人群升级、v2.1 用户隔离、v2.2 产品化界面和 v2.2.1 新用户研究导航`。真实 Supabase 迁移、103 个后端测试、前端构建与 smoke、17 个 Playwright E2E 均已通过，但尚未达到生产上线标准。

本文档根据当前仓库实际内容整理，重点说明你此前已经完成的产品、设计、工程与 AI 编排工作。说明以代码实际实现为主，同时参考已有 PRD、接口文档、Demo 指南、交付清单和完成度快照。

## 1. 项目总览

`AI 论文共写工作台` 面向本科生、硕士生、博士生与科研人员，核心定位是 `证据驱动型研究共创平台`。它不是简单的聊天生成器，而是一套围绕研究项目、学术画像、共享材料、多文档、章节、证据、共写、审查与导出的完整工作流。
...
`你已经把项目从本科课程论文 MVP 升级为接入真实 Supabase 与 OpenAI 兼容 AI 的多文档、章节化研究共创平台，同时保留可信链、共写预览、审查复查、原创实证补强、文献补充回流和导出能力。`
...
- v1 阶段明确本科课程论文写作者；v2.0 已扩展到本科、硕士、博士与科研人员。
...
| v2.0 | 学术项目画像、多文档、章节树、动态 readiness、文档材料隔离、章节 AI、AI 使用记录、文档组装导出和旧项目迁移 |
...
| `/documents` | `AcademicDocumentsPage.jsx` | 多文档、可拖拽章节树、动态 readiness、材料范围、章节共写与组装 |
| `/workspace` | `WorkspacePage.jsx` | 兼容整篇正文编辑、可信链、AI 共写、审查、申诉、版本管理 |
...
- `npm run test:e2e` 使用 Playwright + Chrome 覆盖画像、上传、多文档、章节生成、共写预览、审查复查和导出。
...
- v2.0 正式迁移已在 Supabase 执行，31 个旧项目完成画像、主文档、正文章节和章节版本回填；运行时不再允许 JPA 自动改表。
...
下表保留 v1.x Controller 摘要；v2.0 当前完整接口以 `docs/engineering/v2_academic_workspace_api.md` 为准。
...
当前后端测试报告为 `103` 个测试通过，`0` 失败、`0` 错误、`0` 跳过。文档中出现的 `37 / 49 / 51 / 55 / 60 / 62 / 69 / 74 / 77 / 78 / 92 / 101 / 102` 是对应历史阶段的当时验证数量，不代表当前测试总数。
...
前端当前验证包括生产构建、smoke token 检查和 Playwright Chrome 用户流程；组件级测试仍待补。
...
- 解析页包含解析质量清单和补充说明。
- dist 构建产物存在。
- Playwright E2E 覆盖 v2 学术文档主线和兼容工作台审查导出主线。
...
1. 新建研究项目并设置学术画像与首个文档。
2. 上传文本 / 文件 / 图片。
3. 执行预处理与 AI 解析。
4. 查看解析质量清单并补充材料。
5. 按当前文档与研究范式执行动态 readiness。
6. 构建知识库。
7. 进入学术文档页，切换多文档并查看章节树。
8. 选择文档材料范围，生成或编辑章节。
9. 生成章节共写预览并应用为章节新版本，查看 AI 使用记录。
10. 需要时进入兼容整篇工作台查看可信链、原创补强、审查和复查。
11. 组装并导出 DOCX / PDF。
...
- `ddl-auto: none` 为默认值，Supabase 结构只通过版本化 SQL 迁移维护。
...
- 历史文档保留了 `37 / 49 / 51 / 55 / 60 / 62 / 69 / 74 / 77 / 78 / 92 / 101 / 102` 等阶段测试数量；当前统一验证口径是后端 `103` 个测试通过。
...
## 13. v2.1 / v2.2 / v2.2.1 工程补充（2026-07-22）
...
- 后端：103 个测试通过。
- 前端：11 个 Vitest、生产构建、smoke 和 17 个 Playwright 场景通过。
...
- `user_profiles` 增加 onboarding 状态、版本和完成时间，既有账号迁移为已完成，新注册账号默认为未开始。
- `project_guides` 以 workspaceId 为主键保存当前进度、已有内容、截止日期和引导模式，不重复保存 userId。
- `POST /onboarding/complete` 在事务内创建首个 workspace、画像、文档与 guide；用户重复提交时返回已有项目。
- `GET/PATCH /workspaces/{id}/guide` 沿 workspace 归属校验，并根据材料、解析、readiness、知识库、章节与审查实时派生 7 项任务。
- React 新增 6 步引导、跳过状态、顶栏只读指南和项目概览动态路线设置。
- 真实 Supabase 已登记 `20260722103000`，38 个 workspace 全部回填 guide，RLS 开启且公开角色无授权。
